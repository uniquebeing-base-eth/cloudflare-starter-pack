CREATE OR REPLACE FUNCTION public.record_wallet_shred(_wallet text, _username text, _pack_id text, _items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE COALESCE(item->>'kind', '') NOT IN ('USDM', 'USDT', 'XP', 'CARD', 'FACT')
      ...
      OR length(COALESCE(item->>'title', '')) > 200
      OR length(COALESCE(item->>'sub', '')) > 1000
      OR COALESCE(item->>'rarity', 'Common') NOT IN ('Common', 'Rare', 'Epic', 'Legendary')
      OR CASE
           WHEN item ? 'amount' AND (item->>'amount') IS NOT NULL AND (item->>'amount') <> '' THEN
             CASE
               WHEN (item->>'kind') IN ('USDM','USDT') THEN
                 COALESCE((item->>'amount')::numeric, 0) < 0
                 OR COALESCE((item->>'amount')::numeric, 0) > 20
               WHEN (item->>'kind') = 'XP' THEN
                 COALESCE((item->>'amount')::numeric, 0) < 0
                 OR COALESCE((item->>'amount')::numeric, 0) > 100000
               ELSE false
             END
           ELSE false
         END
  ...
  SELECT
    ...
    item->>'kind',
    left(item->>'title', 200),
    left(COALESCE(item->>'sub', ''), 1000),
    COALESCE(item->>'rarity', 'Common'),
    CASE WHEN item ? 'amount' AND (item->>'amount') IS NOT NULL AND (item->>'amount') <> '' THEN NULLIF(item->>'amount', '')::numeric ELSE NULL END
  FROM jsonb_array_elements(_items) AS item;
